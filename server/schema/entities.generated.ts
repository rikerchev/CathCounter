...
export const ENTITIES: Record<string, EntityDef> = {
  ...
  CustomAd: {
    ...
    columns: [
      ...
      // link, forced, duration_seconds}, ...] (description/link added v3.44,
      // sourced from the merchant's own venues.ad_description/ad_link at
      // attach time; forced added v3.62 — an admin-only demo/test override,
      // toggled per merchant in CustomAds.jsx, that forces this merchant's
      // turn into the live carousel regardless of its real QR-referral
      // eligibility; duration_seconds added v3.65 — this merchant's own turn
      // length in the live carousel, editable per merchant in CustomAds.jsx
      // (see updateMerchantDuration() there) instead of the flat
      // MERCHANT_TURN_SECONDS every merchant used to get — a deliberate
      // manual admin choice (e.g. to reward a merchant bringing in more new
      // registrations with more screen time), never anything automatically
      // derived from the merchant's actual referral count, which stays
      // private — see AdBannerItem.jsx's buildCarouselItems(). Neither
      // override ever touches the merchant's actual
      // bonus_days_per_referral or the merchant_referrals table), in
      // rotation order — same JSON-in-TEXT pattern as country_content/
      // language_content below, not a live join, so the ad-rendering hot
      // path (every page load) never needs an extra fetch. See
